...
const registerValidSW = async (sw_url, config) => {
    try {
...
const checkValidServiceWorker = async (sw_url, config) => {
    ...
    try {
        ...
        if (
            response.status === 404 ||
            (content_type != null && content_type.indexOf('javascript') === -1)
        ) {
